...
import pri.tool.upnp.http.HTTPRequest;
import pri.tool.upnp.UPnP;
...
{
	private final static String TAG = "HTTPMUSocket";
	////////////////////////////////////////////////
	//	Member
	////////////////////////////////////////////////
	...
	public String getLocalAddress()
	{
		...
		while (addrs.hasMoreElements()) {
			...
			Log.e(TAG, "getLocalAddress:" + addr.getHostAddress() + ", host name:" + addr.getHostName());
	...
	public boolean open(String addr,int port, InetAddress bindAddr){
		Log.e(TAG, "open (addr:" + addr + ",port:" + port + ", bindAddr:" + bindAddr.toString());
		try {
...
//			ssdpMultiSock = new MulticastSocket(port);
//			InetAddress ia = InetAddress.getByName(addr);
//			ssdpMultiSock.joinGroup(ia);
		}
		catch (Exception e) {
			Debug.warning(e);
			return false;
	...
	public boolean close()
	{
		...
		//	ssdpMultiSock.leaveGroup(ssdpMultiGroup);
            ssdpMultiSock.close();
			ssdpMultiSock = null;
		}
		catch (Exception e) {
	...
	{
		Log.e(TAG, "send: bindAddr:" + bindAddr + ", bindPort:" + bindPort);
		try {
			...
			if ((bindAddr) != null && (0 < bindPort)) {
				...
				msock = new MulticastSocket(SSDP.PORT);
			}
	...
	public SSDPPacket receive() throws IOException
	{
		byte ssdvRecvBuf[] = new byte[SSDP.RECV_MESSAGE_BUFSIZE];
 		...
 		String locatAddr = getLocalAddress();
// 		Log.e(TAG, "localAddr:" + locatAddr);
		recvPacket.setLocalAddress(locatAddr);

//		byte[] rev = new byte[512];
//		DatagramPacket packet = new DatagramPacket(rev, rev.length);
//		if (ssdpMultiSock != null)
//			ssdpMultiSock.receive(packet); // throws IOException
//		else
